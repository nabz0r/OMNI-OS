...
The HTTP addresses and token files below belong to the explicit external gateway workflow. Installing a native desktop or mobile app alone does not start port 3007 or make its private vault available to browser extensions, MCP clients or another app. In 0.4.0, Work can explicitly open an ephemeral loopback gateway limited to approved named clients and the two provider request routes. It exposes no extension capture or MCP route; the packaged bootstrap still starts with it off. [Work integration contract](WORK.md). See [PLATFORMS.md](PLATFORMS.md).
...
The interface can create a permission for selected confirmed memories, an exact provider base, and a lifetime of at most 24 hours. Retrieve its ID from the owner-authenticated `/api/state` response when configuring another client. For a non-work development installation, configure that application's base URL as `http://127.0.0.1:3007/v1` and its API token from `.omni/runtime/agent-token`. Work mode disables that legacy token: approve a named client, bind its grant with `client_id`, and use the displayed native gateway address or the explicit external core. Never distribute the owner token. [Work](WORK.md). Give it the `x-omni-grant` header only when that memory disclosure is intended. Without the header, the gateway forwards the request with no memory augmentation. A revoked or expired permission fails instead of silently retrying with memory.
