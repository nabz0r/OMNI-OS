...
- The native setup browser command accepts only the fixed `ollama` resource from the main window. It cannot open arbitrary URLs or files, runs no shell and removes credential-like environment variables before launching the external browser.
