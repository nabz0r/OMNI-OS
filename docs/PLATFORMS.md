...
Local request policies and text attachments use the shared Rust/UI implementation on all source targets. Standalone desktop startup can load `OMNI_MANAGED_POLICY_FILE`; the standard mobile bootstrap has no MDM policy provisioning. Native IPC and HTTP requests share the same policy checks. Older revision-pinned installers predate this feature; consult their build record and the [policy guide](POLICIES.md).
...
Android debug signing keys can differ between disposable CI runners. Evaluation APKs therefore do not establish a stable upgrade identity; a pilot release needs a protected, persistent signing key before personal vaults depend on updates.
